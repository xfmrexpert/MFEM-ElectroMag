...
public:
    ...
    {
        // Map from DOF index to (value, boundary_attribute)
        std::map<int, std::vector<std::pair<double, int>>> dof_values;
        ...
        for (const auto& [marker, value] : bcs) {
            ...
            for (int i = 0; i < fespace.GetNBE(); i++) {
                ...
                // Record value for each DOF
                for (int j = 0; j < vdofs.Size(); j++) {
                    int dof = vdofs[j];
                    if (dof < 0) dof = -1 - dof;  // Handle orientation
                    dof_values[dof].push_back({value, attr});
                }
            }
        }
        ...
        for (const auto& [dof, values] : dof_values) {
            if (values.size() > 1) {
                ...
                double first_val = values[0].first;
                bool has_conflict = false;

                for (size_t i = 1; i < values.size(); i++) {
                    if (std::abs(values[i].first - first_val) > tolerance) {
                        has_conflict = true;
                        break;
                    }
                }
                ...
                if (has_conflict) {
                    ...
                    oss << "DOF " << dof << " is constrained by multiple boundaries with DIFFERENT values:\n";
                    for (const auto& [val, attr] : values) {
                        oss << "    Boundary attribute " << attr << ": value = " << val << "\n";
                    }
